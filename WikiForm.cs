...
namespace DataStructuresWiki2
{
    public partial class WikiForm : Form
    {
        ...
        {
            InitializeComponent();
            //createCategoriesTextFile(); used for startup only
        }
        ...
        private void ReadCategoriesFromFile()
        {
            ...
            if (File.Exists(categoriesFileNameIncludingPath))
            {
                try
                {
                    using (StreamReader streamReader = new StreamReader(categoriesFileNameIncludingPath))
                    {
        ...
        {
            if ((string.IsNullOrWhiteSpace(textBoxName.Text)) ||  (string.IsNullOrWhiteSpace(textBoxDefinition.Text)))
            {
                toolStripStatusLabel1.Text = "Name and Definition can not be empty.";
                return false;
            }
        ...
        private void CreateCategoriesTextFile()
        {
            ...
            try
            {
                using (StreamWriter streamWriter = new StreamWriter(Path.Combine(Application.StartupPath, "Categories.txt")))
                {
        ...
        #region TESTING
        //TESTING AND STARTUP ONLY - create binary file for Categories
        private void createCategories()
        {
            ...
            try
            {
                using (Stream stream = File.Open("Categories.txt", FileMode.Create))
                {
                    BinaryFormatter bin = new BinaryFormatter();
                    foreach (string category in validCategories)
                    {
                        bin.Serialize(stream, category);
                    }
                }
            }
            catch (IOException exc)
            {
                MessageBox.Show("createCategories exc: " + exc.Message);
            }
        }
